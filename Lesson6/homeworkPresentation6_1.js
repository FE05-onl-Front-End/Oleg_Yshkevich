...
const serchRichObject = (array, type) => {

  let arraySerchType = array.filter((item) => 
     item.type == type ? item : null)
  
  return arraySerchType.reduce((total , currentValue ) => 
    total.price > currentValue.price ? total : (total = currentValue), 10
  );
}

console.log (serchRichObject(data, 4)) ;